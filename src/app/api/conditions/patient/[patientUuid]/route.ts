...
// GET - Fetch conditions for a patient
export async function GET(
  ...
  try {
    ...
    const url = `${API_BASE}/condition/patient/${patientUuid}`
    console.log('[Condition Proxy] Fetching conditions:', url)
    ...
    console.log('[Condition Proxy] Fetch response status:', response.status)
    ...
    console.error('[Condition Proxy] Error fetching conditions:', error)
    return NextResponse.json(
      { status: 'error', message: error.message || 'Failed to fetch conditions', data: [] },
      { status: 500 }
    )
  }
}
