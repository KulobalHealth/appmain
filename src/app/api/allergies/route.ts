...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE}/allergy`
    console.log('[Allergy Proxy] Adding allergy:', url)
    ...
    console.log('[Allergy Proxy] Add response status:', response.status)
    ...
    console.error('[Allergy Proxy] Error adding allergy:', error)
    return NextResponse.json(
      { status: 'error', message: error.message || 'Failed to add allergy' },
      { status: 500 }
    )
  }
}
