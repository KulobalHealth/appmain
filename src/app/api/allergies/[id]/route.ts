...
export async function DELETE(
  ...
  try {
    ...
    const url = `${API_BASE}/allergy/${id}`
    console.log('[Allergy Proxy] Deleting allergy:', url)
    ...
    console.log('[Allergy Proxy] Delete response status:', response.status)
    ...
    console.error('[Allergy Proxy] Error deleting allergy:', error)
    return NextResponse.json(
      { status: 'error', message: error.message || 'Failed to delete allergy' },
      { status: 500 }
    )
  }
}
